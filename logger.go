...
func (r *Info) Output() string {
	msg := fmt.Sprintf(r.format, r.Id, r.Pid, r.Time, r.Level, r.Message)
	return msg
}
...
func New(args ...interface{}) (*Logger, error) {
	...
	return &Logger{Module: module, worker: newWorker, Pid: os.Getpid()}, nil
}
...
func (l *Logger) Log(lvl string, message string) {
	var formatString string = "#%d pid(%d) %s ▶ %.3s %s"
	info := &Info{
